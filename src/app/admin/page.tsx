...
// ---- API CALL ----
async function fetchUsers(page: number, q?: string, limit = 6): Promise<PaginatedUsers> {
...
const { data, isLoading, isFetching } = useQuery<PaginatedUsers, Error>({
  ...
  return (
    <RoleGuard allowedRoles={["SUPER_ADMIN", "ADMIN_STORE"]}>
              ...
              <>
                ...
                <div
                  className={`overflow-x-auto transition-transform duration-300 ease-out ${
                    isFetching && slideDir === "left" ? "-translate-x-6 opacity-80" : ""
                  } ${isFetching && slideDir === "right" ? "translate-x-6 opacity-80" : ""}`}
                >
                  <UserTable
                    users={data?.data ?? []}
                    // hanya pass handler kalau SUPER_ADMIN, dan beri flag canManage
                    onEdit={role === "SUPER_ADMIN" ? (u) => setEditing(u) : undefined}
                    onDelete={role === "SUPER_ADMIN" ? (u) => setConfirmDelete(u) : undefined}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-slate-700" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M12.707 16.293a1 1 0 01-1.414 0l-5-5.875a1 1 0 010-1.287l5-5.875a1 1 0 011.507 1.287L9.586 9H16a1 1 0 110 2H9.586l3.627 4.293a1 1 0 01-.506 1.0z" clipRule="evenodd" />
                      </svg>
                    </button>
                    ...
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-slate-700" viewBox="0 0 20 20" fill="currentColor">
                        <path fillRule="evenodd" d="M7.293 3.707a1 1 0 011.414 0l5 5.875a1 1 0 010 1.287l-5 5.875a1 1 0 01-1.507-1.287L10.414 11H4a1 1 0 110-2h6.414L7.2 5.0a1 1 0 01.093-1.293z" clipRule="evenodd" />
                      </svg>
                    </button>
                  </div>
                </div>
