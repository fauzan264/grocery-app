...
function ProductsInner() {
  ...
  const deleteBulk = async (ids: string[]) => {
    if (!confirm(`Hapus ${ids.length} produk terpilih? Ini tidak bisa di-undo.`)) return;
    try {
        ...
        <div className="flex items-center gap-4">
          <div className="rounded-full p-3 bg-blue-600 text-white inline-flex items-center justify-center shadow">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
              ...
              <path d="M7 11h10v8a1 1 0 01-1 1H8a1 1 0 01-1-1v-8z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          <div>
            <h1 className="text-2xl font-semibold text-slate-800">Product Management</h1>
            <p className="text-sm text-slate-600">Manage product catalog — add, filter, and remove products.</p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
          ...
          <div className="flex items-center gap-2 bg-white rounded shadow px-3 py-2">
            ...
            <select
              ...
              onChange={(e) => setFilters((prev) => ({ ...prev, categoryId: e.target.value || undefined, page: 1 }))}
          ...
          <div className="flex items-center gap-3">
            <div className="text-right text-sm">
              ...
              <div className="text-lg font-semibold text-slate-700">{totalProducts}</div>
            </div>
            <button
        ...
        <div className="rounded shadow overflow-hidden bg-white">
          <div className="flex items-center justify-between px-4 py-3 border-b">
            <div className="flex items-center gap-3">
              ...
              {isFetching && <div className="text-xs text-slate-700/60">Load...</div>}
            </div>
            <div className="text-sm text-slate-700/70">Hal {data?.meta?.page ?? filters.page} / {data?.meta?.totalPages ?? 1}</div>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center text-slate-700">
              ...
              <div className="text-sm mb-4">Tambahkan produk baru menggunakan tombol "Create Product".</div>
              <button onClick={() => router.push("/admin/products/create")} className="px-4 py-2 rounded bg-blue-600 text-white">Buat Produk</button>
            </div>
          ) : (
            <>
              ...
              <div className="p-4 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="text-sm text-slate-700">Displaying {data?.items?.length ?? 0} from {totalProducts} products</div>

                <div className="flex items-center gap-2">
                  <button
                    aria-label="previous page"
                    disabled={!data || (data?.meta?.page ?? 1) === 1 || isFetching}
                  ...
                  <div className="text-gray-800 px-4">Page {data?.meta?.page ?? filters.page} of {data?.meta?.totalPages ?? 1}</div>

                  <button
                    aria-label="next page"
                    disabled={!data || (data?.meta?.page ?? 1) >= (data?.meta?.totalPages ?? 1) || isFetching}
